...
class Timer(Component):
    ...
    class _TimerProcess:
        def __init__(self, timer):
            self.timer = timer
            self.env = timer.platform.simulation.engine
            self._lastTime = self.env.now

        def eventLoop(self):
            while True:
                yield self.env.timeout(self.timer._period)
    ...
    def start(self, period, repeat=False):
        ...
        self._repeat = repeat
        self.env = self.platform.simulation.engine
        self._process = self.env.process(self._timerProcess())
    ...
    def _timerProcess(self):
        self._lastTime = self.env.now
        while True:
            yield self.env.timeout(self._period)
